...
Project Shortcut Dock 是給 Windows 使用者的桌面捷徑工具。你可以把專案資料夾或常用檔案拖進去，之後雙擊就開、右鍵就進終端機、需要 AI CLI 時也能直接從目前位置啟動。它不會搬動你的檔案，不會偷改你的專案，也不會假裝自己是第二個檔案總管。它只做一件事：讓你不用一直瘋狂點擊找檔案。
...
```text
ProjectShortcutDock-Setup-0.2.0.exe
```
...
```text
ProjectShortcutDock-0.2.0-win-x64.zip
```
...
5. 想調整辨識色時按色點，直接用 RGB 選色器挑色。
6. 雙擊捷徑開啟檔案或資料夾。
7. 右鍵捷徑使用更多操作。
8. 點齒輪調整樣式、視窗模式、語系、Shell、是否隨 Windows 自動啟動。
9. 不想看到它時，右上方打叉隱藏到系統匣就好。
...
```text
artifacts\ProjectShortcutDock-Setup-0.2.0.exe
artifacts\ProjectShortcutDock-0.2.0-win-x64.zip
```
